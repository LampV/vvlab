...
#        self.steps_beyond_done = None
    ...
    def reset(self, seed):
        ...
        self.state = [self.np_random.uniform(low=-self.cell_radius,
                                             high=self.cell_radius),
    ...
    def step(self, action):
        ...
        state = self.state
        reward = 0
        x_ue_1, y_ue_1, x_ue_2, y_ue_2, \
            pt_serving, pt_interferer, f_n_bs1, f_n_bs2 = state
        ...
        if (action != -1):  # optimal
            ...
            if (power_command_l == 0):
                pt_serving *= 10**(-0.03/10.)
            else:
                pt_serving *= 10**(0.03/10.)

            if (power_command_b == 0):
                pt_interferer *= 10**(-0.03/10.)
            else:
                pt_interferer *= 10**(0.03/10.)
        ...
        received_power_ue2, interference_power_ue2, \
            received_ue2_sinr = self._compute_rf(
                    x_ue_2, y_ue_2, pt_serving, pt_interferer,
                    is_ue_2=True)
        ...
        # Did we find a FEASIBLE NON-DEGENERATE solution?
               ...
               (pt_interferer <= self.max_tx_power_interference) and \
               (pt_interferer >= 0) and (received_sinr >= self.min_sinr) and \
               (received_ue2_sinr >= self.min_sinr) and \
               (received_sinr >= self.sinr_target) and \
               (received_ue2_sinr >= self.sinr_target)
        abort = (pt_serving > self.max_tx_power) or \
                (pt_interferer > self.max_tx_power_interference) or \
                (received_sinr < self.min_sinr) or \
                (received_ue2_sinr < self.min_sinr) or \
                (received_sinr > 70) or (received_ue2_sinr > 70)
        # or (received_sinr < 10) or (received_ue2_sinr < 10)
...
#        (pt_serving <= self.max_tx_power) , (pt_serving >= 0) , \
...
#        10*np.log10(LA.norm(h, ord=2))))
    ...
    # https://ieeexplore-ieee-org.ezproxy.lib.utexas.edu/stamp/
    # stamp.jsp?tp=&arnumber=7522613
